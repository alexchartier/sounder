...
if __name__ == '__main__':
    ...
    flist = get_freq_list(os.path.join(op.datadir, op.freq_list_fname))
    freqs = flist.values()
    ...
    srn = data.get_properties('hfrx', sample=idx)['sample_rate_numerator']
    srd = data.get_properties('hfrx', sample=idx)['sample_rate_denominator']
    sr = srn / srd
    ...
    while True:
        ...
        time = datetime.utcfromtimestamp(idx / sr)
        diff = shift_secs_ext - time.second
        diff[diff >= 60] -= 60
        min_diff = diff[diff >= 0].min()
        idx += min_diff * np.float(sr)
        time = datetime.utcfromtimestamp(idx / sr)
        diff = shift_secs - time.second
        ...
        try:
            ...
            print('Freq: %02.2f: Max. value: %02.2f dB at %2.1f m/s, %2.1f km'\
                 % (tune_freq, M.max(), dop_vel[maxind[0]], rg[maxind[1]]))
            ...
            if op.plot:
                ...
                dirn = os.path.join(plotdir, time.strftime('%Y%m%d'))
                try:
                    os.makedirs(dirn)
                except:
                    None
            ...
            if op.save:
                ...
                if np.any(sigind):
                    ...
                    M = csr_matrix(M)
                    dirn = os.path.join(savedir, time.strftime('%Y%m%d'))
                    try:
                        os.makedirs(dirn)
                    except:
                        None
                    spec_fname_t = time.strftime('%2.2fMHz' % tune_freq + '_%H%M%S.%f.pkl')
                    with open(os.path.join(dirn, spec_fname_t), 'wb') as f:
                        pickle.dump(M, f)
